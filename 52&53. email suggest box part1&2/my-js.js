$(function () {
   ...
   let theSummary = 0;
   $('.summary tr td:last-child:not(:first, :last)').each(function () {
      // do this if text of this is number only else skip this and move to next td
      if (!isNaN($(this).text())) theSummary += parseFloat($(this).text());
      $('.the-total').text(theSummary);
   });

   // start auto change content (changing notes)
   (function autoChange() {
            ...
            $(this)
               ...
               .fadeIn(1000, function () {
                  $(this).fadeOut(1000, function () {
                     ...
                     $('.ticker-list li').eq(0).fadeIn(1000).addClass('active');
                     autoChange();
                  });
               });
         }
   ...
   $('.tabs-list li').on('click', function () {
      ...
      $(thisLi.data('content')).fadeIn(1000);
   });
   //switch tabs view
   ...
   // email suggest box
   let emailProviders = ['gmail.com', 'hotmail.com', 'outlook.com', 'yahoo.com'],
      finalString = '';
   ...
   $('.email-suggest').on('keyup', function () {
      ...
      // if ($('.email-suggest').val().includes('@gmail') || $('.email-suggest').val().includes('@hotmail') || $('.email-suggest').val().includes('@outlook') || $('.email-suggest').val().includes('@yahoo') || $('.email-suggest').val().includes('@')) {
      if ($('.email-suggest').val().includes('@')) {
         for (let mm = 0; mm < emailProviders.length; mm++) {
            finalString += '<li>' + $(this).val() + '</li>';
         }
      } else {
         for (let mm = 0; mm < emailProviders.length; mm++) {
            finalString += '<li>' + $(this).val() + '@' + emailProviders[mm] + '</li>';
         }
      }
   ...
   // another solution for email suggest box
   // let emailProviders = ['gmail.com', 'hotmail.com', 'outlook.com', 'yahoo.com'];
   // $('.email-suggest').on('keyup', function () {
   //    if (!$(this).next().is('.suggest-box')) {
   //       $('<ul class="suggest-box"></ul>').insertAfter($(this));
   //       for (let m = 0; m < emailProviders.length; m++) {
   //          $(`<li></li>`).appendTo('.suggest-box');
   //       }
   //    }
   //    $('.suggest-box li').each(function (index) {
   //       $(this).text($('.email-suggest').val() + '@' + emailProviders[index]);
   //    });
   // });
   // $('body').on('click', '.suggest-box li', function () {
   //    $('.email-suggest').val($(this).text());
